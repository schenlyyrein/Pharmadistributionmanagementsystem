...
export function StockManagement() {
  ...
  const lowStockItems = mockStock.filter(item => item.status === "low" || item.status === "critical");
  const criticalItems = mockStock.filter(item => item.status === "critical");

  const getStatusColor = (status: string) => {
    switch (status) {
      case "healthy": return "bg-[#00A3AD] text-white";
      case "low": return "bg-[#F97316] text-white";
      case "critical": return "bg-[#DC2626] text-white";
      case "overstock": return "bg-[#1A2B47] text-white";
  ...
  return (
    <div className="p-4 lg:p-8 space-y-8 bg-[#F8FAFC]">
      ...
      <div>
        ...
        <p className="text-[#6B7280]">Real-time inventory levels across warehouse locations</p>
      </div>
        ...
        <Card className="bg-white border-[#111827]/10 shadow-sm">
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-2xl font-bold text-[#111827]">{mockStock.length}</div>
                <div className="text-sm text-[#6B7280]">Total SKUs Tracked</div>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white border-[#F97316]/20 shadow-sm">
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-2xl font-bold text-[#F97316]">{lowStockItems.length}</div>
                <div className="text-sm text-[#6B7280]">Low Stock Alerts</div>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white border-[#DC2626]/20 shadow-sm">
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-2xl font-bold text-[#DC2626]">{criticalItems.length}</div>
                <div className="text-sm text-[#6B7280]">Critical - Restock Now</div>
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium ${
                    item.status === "critical" ? "bg-[#DC2626] text-white" : "bg-[#F97316] text-white"
                  }`}>
                    {getStatusIcon(item.status)}
                    {item.status === "critical" ? "Restock Needed" : "Low Stock"}
                  </span>
                </div>

                <div className="grid grid-cols-3 gap-4 mb-3">
                  <div>
                    <div className="text-xs text-[#6B7280] mb-1">Current Stock</div>
                    <div className={`text-lg font-bold ${
                      item.status === "critical" ? "text-[#DC2626]" : "text-[#F97316]"
                    }`}>
                      {item.currentStock.toLocaleString()}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-[#6B7280] mb-1">Min Stock Level</div>
                    <div className="text-lg font-semibold text-[#111827]">
                      {item.minStock.toLocaleString()}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-[#6B7280] mb-1">Shortage</div>
      ...
      <Card className="bg-white border-[#111827]/10 shadow-sm">
        <CardContent className="pt-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <Label className="text-[#6B7280] mb-2 block">Warehouse Location</Label>
            ...
            <div>
              <Label className="text-[#6B7280] mb-2 block">Stock Status</Label>
            ...
            <div className="flex items-end">
              <Dialog open={showTransferDialog} onOpenChange={setShowTransferDialog}>
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle className="text-[#111827]">Stock Transfer Request</DialogTitle>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b-2 border-[#1A2B47]">
                  ...
                  <th className="text-left py-4 px-4 text-sm font-semibold text-[#111827] bg-[#F8FAFC]">Product</th>
                  <th className="text-left py-4 px-4 text-sm font-semibold text-[#111827] bg-[#F8FAFC]">Location</th>
                  <th className="text-left py-4 px-4 text-sm font-semibold text-[#111827] bg-[#F8FAFC]">Zone/Aisle/Bin</th>
                  <th className="text-right py-4 px-4 text-sm font-semibold text-[#111827] bg-[#F8FAFC]">Current</th>
                  <th className="text-right py-4 px-4 text-sm font-semibold text-[#111827] bg-[#F8FAFC]">Min/Max</th>
                  <th className="text-left py-4 px-4 text-sm font-semibold text-[#111827] bg-[#F8FAFC]">Status</th>
                  <th className="text-left py-4 px-4 text-sm font-semibold text-[#111827] bg-[#F8FAFC]">Last Restocked</th>
                </tr>
              </thead>
              <tbody>
                {filteredStock.map((item) => (
                  ...
                  >
                    <td className="py-4 px-4">
                      <span className="font-mono text-[#00A3AD] font-semibold">{item.sku}</span>
                    </td>
                    <td className="py-4 px-4 text-[#111827] font-medium">{item.name}</td>
                    <td className="py-4 px-4 text-sm text-[#6B7280]">{item.location}</td>
                    ...
                    <td className="py-4 px-4 text-sm text-[#6B7280]">{item.lastRestocked}</td>
                  </tr>
                ))}
              </tbody>
            </table>
