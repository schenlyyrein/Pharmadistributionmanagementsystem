...
export default function ExpirationDashboard() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (result?.length > 0) {
        showToast(
          `✓ Released stock from ${result.length} expired PO(s)`,
        );
      } else {
    ...
    <div style={s.page}>
      ...
      <div style={s.statsRow}>
        {[
          ...
          <div
            key={stat.label}
            style={{ ...s.statCard, background: stat.bg }}
          >
            <strong
              style={{
                fontSize: 32,
